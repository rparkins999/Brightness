...
class Slider extends View {
    ...
    private boolean mEnabled; // true if can set value using slider
    ...
    private float mVisualValue; // current displayed value
    private final FloatProperty<Slider> VISUAL_VALUE =
        new FloatProperty<Slider>("thumbPos") {
            @Override
            public void setValue(Slider object, float value) {
                object.setThumbPos(value);
            }
            @Override
            public Float get(Slider object) {
                return object.mVisualValue;
            }
        };
    ...
    private void updateThumbAndTrackPos(int w, int h, boolean paddingChanged) {
        ...
        setThumbPos(mValue / (float)(mMax - mMin));
    }
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int mw = resolveSizeAndState(dw, widthMeasureSpec, 0);
        int mh = resolveSizeAndState(dh, heightMeasureSpec, 0);
    ...
    public void setValue(int value, boolean animate) {
        ...
        int range = mMax - mMin;
        final float vStart = range > 0 ? (mValue - mMin) / (float) range : 0;
        final float vEnd = range > 0 ? (value - mMin) / (float) range : 0;
        mValue = value;
        if (animate) {
            // should be able to re-use by doing
            // animator.setValues(vStart, vEnd);
            final ObjectAnimator animator =
                ObjectAnimator.ofFloat(this, VISUAL_VALUE, vStart, vEnd);
